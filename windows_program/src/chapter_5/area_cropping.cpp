...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
    {
        ...
        // hRgnClip = CreateRectRgn(0, cyClient/3, cxClient/2 , 2*cyClient/3);
        // hRgnClip = CreateEllipticRgn(cxClient/3, 0, 2*cxClient/3, cyClient/2);
        // FillRgn(hdc,hRgnClip,(HBRUSH)GetStockObject(BLACK_BRUSH));

        SelectClipRgn(hdc, hRgnClip);

        fRaius = _hypot(cxClient/2.0,cyClient/2.0);

        for (fAngle = 0.0; fAngle < TWO_PI; fAngle += TWO_PI/360) {
            MoveToEx(hdc, 0, 0, NULL);
            // 0.5 四舍五入
            LineTo(hdc, (int)(fRaius * cos(fAngle) + 0.5), 
                   (int)(fRaius * sin(fAngle) + 0.5));
        }
